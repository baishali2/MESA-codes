...
! ***********************************************************************
      ...
      module run_star_extras
         ...
         real(dp), allocatable ,save:: t_power(:), f_power(:), t_force(:), f_force(:)
      ...
      subroutine holgado_simulation_force(id, ierr, M, a_in, a_out)
         ...
         integer :: filesize
         logical :: file_exists
         logical, save :: force_loaded = .false.
         logical, save :: power_loaded = .false.
         
         ! force reading variables
         integer :: ios, i
         real(dp) :: dummy
         integer :: n_power = 1278
         integer :: n_force = 1278
         real(dp) :: p_curr, f_curr
         real(dp) :: t_shift 
         ...
         ! Read ONLY once
         if (.not. power_loaded) then

            open(newunit=my_unit, file='Power_phi3_vs_time.dat', &
               status='old', action='read', iostat=ios)
            if (ios /= 0) then
               write(*,*) 'Error: cannot open Power_phi3_vs_time.dat'
               ierr = 1
               return
            end if
            ...
            do i = 1, n_power
               ...
               if (ios /= 0) then
                  write(*,*) 'Error reading power table at line ', i
                  ierr = 1
                  close(my_unit)
                  return
               end if
            ...
            write(*,*) 'power table loaded successfully.'
         end if

         ! read force data from file
         ! Read ONLY once
         if (.not. force_loaded) then
            ...
            do i = 1, n_force
               read(my_unit,*,iostat=ios) dummy, t_force(i), f_force(i)
               if (ios /= 0) then
                  write(*,*) 'Error reading force table at line ', i
                  ierr = 1
                  close(my_unit)
                  return
               end if
            ...
            write(*,*) 'force table loaded successfully.'
         end if
         ...
         if (.not. file_exists .or. filesize == 0) then
            write(my_unit,'(a)') 'model   zone        R(cm)           term1      term2     term3      term4      term5        dEorb           m               rho                 f         fd_arr       v       fd*v(edot_rk4)      Eorb          M     edot'
         end if
         ...
         ! shifting the time by 20.3 days, since that is when the 3d orbit reaches 122 orbital radius
         t_shift= 20.3*86400   ! time shifted in seconds

         !call interpolate_force(s%time+t_shift , n_power, t_power, f_power, p_curr)  ! should be enough
         call interpolate_force(s%time+t_shift , n_force, t_force, f_force, f_curr)

         fd_arr =  f_curr            !dyne (g cm s^-2) 
         edot = fd_arr*v                !erg/s  !power from 3d simulation oscillates ! edot is negative
         ...
         term5 = fd_arr*v + term4  ! not edot anymore, the injected energy and orbital evolution term is different

         f = term1*term2*term5     !cm s^-1(da/dt)  ! already negative, no need for another negative sign  
            
         write(my_unit,'(i6,1x,i6,1x,13(3x,es12.4))') &
         s%model_number, k, s%R(k), term1,term2,term3,term4,term5, dEorb, s%m(k), s%rho_face(k), f, fd_arr, v, fd_arr*v, Eorb, M , edot
      ...
      subroutine energy_injection_bronner(id, ierr, edot_in, nr_in, rzones_in, a_in, r1_in, r2_in)
         ...
         do i = 1, nr_in
            ...
            if (s%R(k) .ge. s%R(rzones_in(nr_in)) .and. s%R(k) .lt. a_in) then 
               kernel(k) = EXP(-((a_in - s%R(k))/(a_in - s%R(rzones_in(nr_in))))**2)
            else if (s%R(k) .ge. a_in .and. s%R(k) .le. s%R(rzones_in(1))) then
               kernel(k) = EXP(-((a_in - s%R(k))/(a_in - s%R(rzones_in(1))))**2)
            else
               kernel(k) = 0
            end if
      ...
      subroutine inject_energy(id, ierr)
         ...
         if (.not. allocated(t_force)) then
            allocate(t_force(n_force))
         end if
         if (.not. allocated(f_force)) then
            allocate(f_force(n_force))
         end if
         ...
         if (s%model_number == 1) then
            s%xtra(a_curr) = 122*Rsun !9.8d-1*s%R(1)          !cm
