...
! ***********************************************************************
      ...
      module run_star_extras
         ...
         real(dp) :: mdot_edd, mdot_hyper, mdot_arr, fd_arr, edot, f
         ...
         real(dp), allocatable ,save:: t_force2(:), f_force2(:), t_force1(:), f_force1(:)
      ...
      subroutine holgado_prescription(id, ierr, M, a_in, a_out)
         ...
         ! removing loop
         k=azone_temp
         v = SQRT(standard_cgrav*(M + s%m(k))/s%R(k))   !cm/s
      ...
      subroutine holgado_simulation_force(id, ierr, M, a_in, a_out)
         ! star variables
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type(star_info), pointer :: s
         ...
         integer :: filesize
         logical :: file_exists
         logical, save :: force_loaded1
         logical, save :: force_loaded2
         real(dp) :: t_switch
         
         ! force reading variables, part 1
         integer :: ios, i
         real(dp) :: dummy
         integer :: n_force1 = 1278
         real(dp) :: f_curr1

         ! force reading variables, part 2
         integer :: n_force2 = 1278
         real(dp) :: f_curr2
         ...
         ! read force data from file(prescription 1)
         ! Read ONLY once
         if (.not. force_loaded1) then

            open(newunit=my_unit, file='F_phi_2_vs_time.dat', &
               status='old', action='read', iostat=ios)
            if (ios /= 0) then
               write(*,*) 'Error: cannot open F_phi_2_vs_time.dat'
               ierr = 1
               return
            end if
            ...
            do i = 1, n_force1
               read(my_unit,*,iostat=ios) dummy, t_force1(i), F_force1(i)
               if (ios /= 0) then
                  write(*,*) 'Error reading force1 table at line ', i
                  ierr = 1
                  close(my_unit)
                  return
               end if
            ...
            write(*,*) 'Force table 1 loaded successfully.'
         end if


         ! read force data from file(prescription 2)
         ! Read ONLY once
         if (.not. force_loaded2) then

            open(newunit=my_unit, file='F_phi_vs_time.dat', &
               status='old', action='read', iostat=ios)
            if (ios /= 0) then
               write(*,*) 'Error: cannot open F_phi_vs_time.dat'
               ierr = 1
               return
            end if
            ...
            do i = 1, n_force2
               read(my_unit,*,iostat=ios) dummy, t_force2(i), F_force2(i)
               if (ios /= 0) then
                  write(*,*) 'Error reading force2 table at line ', i
                  ierr = 1
                  close(my_unit)
                  return
               end if
            ...
            write(*,*) 'Force table 2 loaded successfully.'
         end if
         ...
         if (.not. file_exists .or. filesize == 0) then
            write(my_unit,'(a)') 'model   zone        R(cm)           term1      term2     term3      term4      term5        dEorb           m               rho                 f         fd_arr       v         Eorb          M'
         end if
         ...
         t_switch = 97*86400  ! time in seconds to switch, 70 days
         
         if (s%time <= t_switch) then
            call interpolate_force(s%time, n_force1, t_force1, f_force1, f_curr1)
            fd_arr =  -f_curr1   !fd_arr is positive
            edot = -fd_arr*v     ! edot is negative
         else if (s%time .ge. t_switch) then
            call interpolate_force(s%time, n_force2, t_force2, f_force2, f_curr2)
            fd_arr =  -f_curr2
            edot = -fd_arr*v
         end if                   
         ...
         term1 =  2*s%R(k)**2/(standard_cgrav*M*s%m(k))
         ...
         term4 =  4*pi*standard_cgrav*M*( s%R(k)* s%rho(k) * s%u(k)/2  - term3)
         term5 = edot + term4

         f = term1*term2*term5     !cm s^-1(da/dt)  ! already negative, no need for another negative sign  
            
         write(my_unit,'(i6,1x,i6,1x,12(3x,es12.4))') &
         s%model_number, k, s%R(k), term1,term2,term3,term4,term5, dEorb, s%m(k), s%rho_face(k), f, fd_arr, v, Eorb, M 
      ...
      subroutine smooth_profile_holgado(id, ierr, M, a_in, a_out)
         ...
         do k= 1, n_smooth
            v_smooth(k) = SQRT(standard_cgrav*(M + m_smooth(k))/( R_smooth(k)))
         end do
         ...
         do k = 1, n_smooth
            Ra_smooth(k) = (2*standard_cgrav*M)/(v_smooth(k)**2)
            ...
            dEorb_smooth(k) = ((standard_cgrav*M)/(2*R_smooth(k))) * ((m_smooth(k)/R_smooth(k)) - 4*pi*(R_smooth(k)**2)*rho_boundary_smooth(k))
      ...
      subroutine bronner_prescription(id, ierr, M, a_out)
         ...
         Ra_br = 2*standard_cgrav*M/(v_rel**2)                 !cm  !different than the one in bronner
      ...
      subroutine hl_and_energy(id, ierr, M, a_in)
         ...
         Eorb =  standard_cgrav*M*(s%m(k))/(2.0d0*s%R(k))
         Eorb =  Eorb + standard_cgrav*M*E_outer  ! erg

         dEorb = (standard_cgrav*M/(2*s%R(k)))*((s%m(k)/s%R(k)) + 4*pi*(s%R(k)**2)*s%rho_face(k)) 
      ...
      subroutine kim2007(id, ierr, n, M)
         ...
         do k = 1, n
            ...
            fd_arr = fd_arr*(4*pi*s%rho(k)*((standard_cgrav*M)**2)/(v**2))
         end do
      ...
      ! inject energy with a half-gaussian profile in the energy injection zone lying inside the envelope when the companion is outside
      ...
      subroutine inject_energy(id, ierr)
         ...
         if (.not. allocated(t_force1)) then
            allocate(t_force1(n_force1))
         end if
         if (.not. allocated(f_force1)) then
            allocate(f_force1(n_force1))
         end if
         if (.not. allocated(t_force2)) then
            allocate(t_force2(n_force2))
         end if
         if (.not. allocated(f_force2)) then
            allocate(f_force2(n_force2))
         end if
         ...
         if (s%model_number == 1) then
            s%xtra(a_curr) = 124*Rsun !9.8d-1*s%R(1)          !cm  ! starting outside the star
         ...
         if (prescription == 1) then  
            if (s%x_ctrl(19) == 1) then                         ! smoothing turned on
               ...
               call holgado_simulation_force(id, ierr, s%xtra(M_ns_curr), s%xtra(a_curr), a_next)
            end if
         else if (prescription == 2) then
      ...
      subroutine data_for_extra_history_columns(id, n, names, vals, ierr)
         ...
         vals(23) = s%R(azone)
         vals(24) = decay_coeff
         vals(25) = sqrt(2*standard_cgrav*s% m(azone)/s% R(azone))
         vals(26) = s%u(azone)
